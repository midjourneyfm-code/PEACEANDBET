  ...
  if (subaction === 'continue') {
    ...
    console.log(`✅ ${interaction.user.tag} passe au tour ${game.round} (x${nextRoundData.multiplier})`);
      ...
      if (subaction === 'tile') {
        ...
        // 🎮 PIERRE FEUILLE CISEAUX - Accepter/Refuser
    if (action === 'pfc' && (params[0] === 'accept' || params[0] === 'decline')) {
      const subaction = params[0];
      const challengerId = params[1];
      const challengedId = params[2];
      const amount = parseInt(params[3]);
      
      // Vérifier que c'est bien le joueur challengé qui clique
      if (interaction.user.id !== challengedId) {
        return interaction.reply({ 
          content: '❌ Seul le joueur défié peut accepter ou refuser !', 
          ephemeral: true 
        });
      }
      
      const gameId = `${challengerId}_${challengedId}`;
      
      if (!activePFCGames.has(gameId)) {
        return interaction.reply({ 
      ...
      if (subaction === 'decline') {
        activePFCGames.delete(gameId);
        
        const declinedEmbed = new EmbedBuilder()
          .setColor('#FF0000')
          .setTitle('🎮 DÉFI REFUSÉ')
          .setDescription(
            `**<@${challengedId}>** a refusé le défi de **<@${challengerId}>** !\n\n` +
            `💰 Montant : ${amount}€\n` +
            `😔 Dommage...`
          );
      ...
      const challenged = await getUser(challengedId);
      
      // Vérifier encore les soldes
      if (challenger.balance < amount) {
        return interaction.reply({ 
          content: `❌ <@${challengerId}> n'a plus assez d'argent !`, 
          ephemeral: true 
        });
      }
      ...
      if (challenged.balance < amount) {
        return interaction.reply({ 
          content: `❌ Vous n'avez plus assez d'argent !`, 
          ephemeral: true 
        });
      }
      
      challenger.balance -= amount;
      challenged.balance -= amount;
      await challenger.save();
      await challenged.save();
      
      // Créer les boutons de choix pour les 2 joueurs
      const choicesEmbed = new EmbedBuilder()
        .setColor('#FFD700')
        .setTitle('🎮 PIERRE FEUILLE CISEAUX - EN COURS')
        .setDescription(
          `**<@${challengerId}>** 🆚 **<@${challengedId}>**\n\n` +
          `💰 **Mise :** ${amount}€ chacun (déjà déduit)\n` +
          `🏆 **Gain potentiel :** ${amount * 2}€\n\n` +
          `Faites votre choix en messages privés ! ⏰`
        )
        .setFooter({ text: 'Vous avez 30 secondes pour choisir' });
      
      await interaction.update({ embeds: [choicesEmbed], components: [] });
      
      // Envoyer les choix en DM
      const choiceRow = new ActionRowBuilder()
        .addComponents(
          new ButtonBuilder()
            .setCustomId(`pfc_choice_rock_${gameId}_${amount}`)
            .setLabel('🪨 Pierre')
            .setStyle(ButtonStyle.Primary),
          new ButtonBuilder()
            .setCustomId(`pfc_choice_paper_${gameId}_${amount}`)
            .setLabel('📄 Feuille')
            .setStyle(ButtonStyle.Primary),
          new ButtonBuilder()
            .setCustomId(`pfc_choice_scissors_${gameId}_${amount}`)
            .setLabel('✂️ Ciseaux')
            .setStyle(ButtonStyle.Primary)
        );
      
      const choiceDMEmbed = new EmbedBuilder()
        .setColor('#FFD700')
        .setTitle('🎮 Faites votre choix !')
        .setDescription(`Choisissez : 🪨 Pierre, 📄 Feuille ou ✂️ Ciseaux`)
        .setFooter({ text: '⏰ 30 secondes pour choisir' });
      
      try {
        const challengerUser = await client.users.fetch(challengerId);
        const challengedUser = await client.users.fetch(challengedId);
        
        await challengerUser.send({ embeds: [choiceDMEmbed], components: [choiceRow] });
        await challengedUser.send({ embeds: [choiceDMEmbed], components: [choiceRow] });
      } catch (error) {
        // Si DM fermés, rembourser
        challenger.balance += amount;
        challenged.balance += amount;
        await challenger.save();
        await challenged.save();
        
        const errorEmbed = EmbedBuilder.from(choicesEmbed)
          .setColor('#FF0000')
          .setDescription('❌ Impossible d\'envoyer les choix en DM. Défi annulé et remboursé.');
        
        interaction.message.edit({ embeds: [errorEmbed], components: [] }).catch(() => {});
        return;
      }
      
      // Stocker le jeu en attente des choix
      const pfcGame = {
      ...
      activePFCGames.set(gameId, pfcGame);
      
      // Timeout si pas de choix
      setTimeout(async () => {
        if (activePFCGames.has(gameId)) {
          const game = activePFCGames.get(gameId);
          activePFCGames.delete(gameId);
          
          // Rembourser ceux qui n'ont pas choisi
          const challengerChose = game.choices[challengerId];
          const challengedChose = game.choices[challengedId];
          ...
          const timeoutEmbed = new EmbedBuilder()
            ...
            .setDescription(
              `Le défi a expiré !\n\n` +
              `${challengerChose ? '✅' : '❌'} <@${challengerId}> ${challengerChose ? 'a choisi' : 'n\'a pas choisi'}\n` +
              `${challengedChose ? '✅' : '❌'} <@${challengedId}> ${challengedChose ? 'a choisi' : 'n\'a pas choisi'}\n\n` +
              `Les joueurs n'ayant pas choisi ont été remboursés.`
            );
    ...
    // 🎮 CHOIX PFC
    if (action === 'pfc' && params[0] === 'choice') {
      const choice = params[1]; // rock, paper, scissors
      const gameId = params[2];
      const amount = parseInt(params[3]);
      
      if (!activePFCGames.has(gameId)) {
        return interaction.reply({ 
          content: '❌ Ce jeu n\'existe plus ou a expiré.', 
          ephemeral: true 
        });
      }
      ...
      if (interaction.user.id !== game.challenger && interaction.user.id !== game.challenged) {
        return interaction.reply({ 
          content: '❌ Vous ne faites pas partie de ce jeu !', 
          ephemeral: true 
        });
      }
      ...
      await interaction.reply({ 
        content: `✅ Vous avez choisi **${choiceEmoji[choice]}** !`, 
        ephemeral: true 
      });
      
      // Vérifier si les 2 ont choisi
      if (game.choices[game.challenger] && game.choices[game.challenged]) {
        activePFCGames.delete(gameId);
        
        const choice1 = game.choices[game.challenger];
        const choice2 = game.choices[game.challenged];
        ...
        if (choice1 === choice2) {
          ...
          winner = game.challenged;
        }
        
        // Mettre à jour les soldes
        const challenger = await getUser(game.challenger);
        const challenged = await getUser(game.challenged);
        ...
        if (winner === 'tie') {
          ...
          resultDescription = 
            `🤝 **ÉGALITÉ !**\n\n` +
            `<@${game.challenger}> : ${choiceEmoji[choice1]}\n` +
            `<@${game.challenged}> : ${choiceEmoji[choice2]}\n\n` +
            `Les mises ont été remboursées.`;
        } else {
          // Donner tout au gagnant
          const winnerUser = winner === game.challenger ? challenger : challenged;
          winnerUser.balance += amount * 2;
          await winnerUser.save();
          
          const loser = winner === game.challenger ? game.challenged : game.challenger;
          const loserChoice = winner === game.challenger ? choice2 : choice1;
          const winnerChoice = winner === game.challenger ? choice1 : choice2;
          
          resultDescription = 
            `🎉 **<@${winner}> GAGNE !**\n\n` +
            `<@${game.challenger}> : ${choiceEmoji[choice1]} ${winner === game.challenger ? '✅' : '❌'}\n` +
            `<@${game.challenged}> : ${choiceEmoji[choice2]} ${winner === game.challenged ? '✅' : '❌'}\n\n` +
            `💰 **<@${winner}>** remporte **${amount * 2}€** (+${amount}€)\n` +
            `💀 **<@${loser}>** perd **${amount}€**`;
        }
        ...
        message.edit({ embeds: [resultEmbed], components: [] }).catch(() => {});
      }
    }
...
if (command === '!mes-combis' || command === '!mc') {
  ...
  message.reply({ embeds: [embed] });
   ...
   // Map pour stocker les défis PFC en attente
  const activePFCGames = new Map();

  if (command === '!pfc' || command === '!chifoumi') {
    const targetUser = message.mentions.users.first();
    const amount = parseInt(args[2]);
    ...
    if (amount < 10) {
      return message.reply('❌ Le montant minimum est de **10€**.');
    }
    
    // Vérifier les soldes
    const challenger = await getUser(message.author.id);
    const challenged = await getUser(targetUser.id);
    
    if (challenger.balance < amount) {
      return message.reply(`❌ Solde insuffisant. Vous avez **${challenger.balance}€**.`);
    }
    
    if (challenged.balance < amount) {
      return message.reply(`❌ <@${targetUser.id}> n'a pas assez d'argent (solde : ${challenged.balance}€).`);
    }
    
    // Vérifier si un défi est déjà en cours
    const gameId = `${message.author.id}_${targetUser.id}`;
    const reverseGameId = `${targetUser.id}_${message.author.id}`;
    
    if (activePFCGames.has(gameId) || activePFCGames.has(reverseGameId)) {
      return message.reply('❌ Un défi est déjà en cours entre vous deux !');
    }
    
    // Créer l'embed du défi
    const challengeEmbed = new EmbedBuilder()
      .setColor('#FF6B00')
      .setTitle('🎮 DÉFI PIERRE FEUILLE CISEAUX')
      .setDescription(
        `**<@${message.author.id}>** défie **<@${targetUser.id}>** !\n\n` +
        `💰 **Mise :** ${amount}€ chacun\n` +
        `🏆 **Gain potentiel :** ${amount}€\n\n` +
        `<@${targetUser.id}>, acceptez-vous le défi ?`
      )
      .addFields(
        { name: '🪨 Pierre', value: 'Bat Ciseaux', inline: true },
        { name: '📄 Feuille', value: 'Bat Pierre', inline: true },
        { name: '✂️ Ciseaux', value: 'Bat Feuille', inline: true }
      )
    ...
    const row = new ActionRowBuilder()
      .addComponents(
        new ButtonBuilder()
          .setCustomId(`pfc_accept_${message.author.id}_${targetUser.id}_${amount}`)
          .setLabel('✅ Accepter')
          .setStyle(ButtonStyle.Success),
        new ButtonBuilder()
          .setCustomId(`pfc_decline_${message.author.id}_${targetUser.id}`)
          .setLabel('❌ Refuser')
          .setStyle(ButtonStyle.Danger)
      );
    ...
    setTimeout(() => {
      if (activePFCGames.has(gameId)) {
        ...
        const expiredEmbed = EmbedBuilder.from(challengeEmbed)
          .setColor('#808080')
          .setDescription(
            `**<@${message.author.id}>** a défié **<@${targetUser.id}>**\n\n` +
            `⏰ **Défi expiré** - Non accepté à temps`
          );
        
        challengeMsg.edit({ embeds: [expiredEmbed], components: [] }).catch(() => {});
      }
    }, 60000);
  }
